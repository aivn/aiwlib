# -*- coding: utf-8 -*-
'''Copyright (C) 2003-2017, 2024 Antov V. Ivanov  <aiv.racs@gmail.com>
Licensed under the Apache License, Version 2.0'''

import os, sys, hashlib, gzip, tempfile
from .mixt import *
#-------------------------------------------------------------------------------
...
def commit(path, add_src_list=[]):
    ...
    open(path+'.src.md5', 'w').write(''.join(time.strftime('%s %%Y.%%m.%%d-%%H:%%M:%%S %s'%(s[6], l), time.gmtime(s[8])) for s, l in zip(statL, md5L)))
    tmp = tempfile.mkstemp()[1]; os.system('tar -Pc '+srcS+' > '+tmp); tar = open(tmp, 'rb').read(); os.remove(tmp)

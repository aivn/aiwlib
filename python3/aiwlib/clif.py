...
Некорректные подстановки {...} оставляются без изменений.
'''
# ввести как то арифметику - если переменная выглядит как число то это число? Вместо str.format использовать f-строки через eval?
    ...
    while pos<len(src_seq):
        ...
        if '#=' in el: scope.__setitem__(*el.split('#=', 1))
            ...
            while 1:  # игнорируем некорректные подстановки путем замены на самих себя
                try: el = el.format(**tmp_scope); break
                except KeyError as ex: ex = eval(str(ex)); tmp_scope[ex] = '{%s}'%ex
            dst_seq.append(el)  
    return dst_seq
CLIF.__doc__ += '\n'+__doc__
#-------------------------------------------------------------------------------    

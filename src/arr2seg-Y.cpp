...
const char *help = "usage: arr2seg-Y options and input arrfiles...\noptions:\n"
	...
	"    -d dx,[dy,]dz --- steps (default from arrayfiles)\n"
	"    -PV x,y --- PV coordinates (default 0,0)\n"
	"    -PP0 x,[y,]z --- first PP coordinates (default from arrayfile)\n"
	"    -s ij --- swap (transpose) axes i and j (for example -s 01), AS A RESULT OF THE TIME AXIS MUST BE ALONG THE X-AXIS!\n"
...
int main(int argc, const char ** argv){
	...
	for(int i=1; i<argc; i++){
		...
		if(opt=="-d" and i<argc-1){ step = atoV(argv[++i]); use_step = true; continue; }
		if(opt=="-PV" and i<argc-1){ PV = atoV(argv[++i]); continue; }
		if(opt=="-PP0" and i<argc-1){ PP0 = atoV(argv[++i]); use_PP0 = true; continue; }
		if(opt=="-s" and i<argc-1){ i++; swaps[sw_count++] = ind(argv[i][0]-48, argv[i][1]-48); continue; }
		...
		if(dim==2){
			...
			segy_write(File(dst.c_str(), "wb"), arr, PV(0,1), (use_PP0?PP0:arr.bmin|0.));
		} else {
			...
			segy_write(File(dst.c_str(), "wb"), arr, PV(0,1), (use_PP0?PP0:arr.bmin));
		}
		std::cout<<src<<" ===> "<<dst<<" [OK]\n";
	}
}

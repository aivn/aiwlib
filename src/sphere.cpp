 ...
 **/
#define INIT_ARR_ZERO  {nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr}
	...
	void arrs_init( int rank ){
		if (rank==0) init_zero_rank();
			...
			if (rank>0){
				...
				uint64_t k = sph_vertex_num(rank);
				...
				for ( uint64_t i = 0; i < PrevN; i++ ){//цикл по всем центральным треугольникам нашей сетки
					...
					if (rank>1) {
						...
						for(uint64_t i = N_2/2+2; i < Prev2VN; i++ ){//Вершины появившиеся на предыдущем уровне рекурсии
							Ind6 cur =vertex_cells[rank-1][i];
		...
		void base_sphere::mass_finish(int rank){
			uint64_t CurN = sph_cells_num(rank);
			uint64_t CurVn = sph_vertex_num(rank)l
				uint64_t PrevN= sph_cells_num(rank-1);
			uint64_t PrevVn= sph_vetex_num(rank-1);
			if (rank!=0){
				if(!normals[rank]){

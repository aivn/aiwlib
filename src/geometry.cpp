...
Vecf<3> trans_vec (const Vecf<3> &r, const Vecf<3> &center, const Vecf<3> &ox, const Vecf<3> &oy, const Vecf<3> &oz){
    if(fabs(ox * (oy%oz)) <1e-16) WRAISE("Transformation is incorrect");
...
struct IntersectFigure: public BaseFigure{
    ...
    virtual Vecf<3> get_min() const { return lchild->get_min() >> rchild->get_min(); };
    virtual Vecf<3> get_max() const { return lchild->get_max() << rchild->get_max(); };
    virtual bool check(const aiv::Vecf<3> &r) const { return lchild->check(r) && rchild->check(r); };
};
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
Vecf<3> rotmov(const aiv::Vecf<3> &r, const aiv::Vecf<3> &center, const aiv::Vecf<3> &n, double phi){
    if (n.abs() < 1e-16) WRAISE("Transformation is incorrect");
    Vecf<3> r0 = r-center;
...
//------------------------------------------------------------------------------
Vecf<3> rotmov_back(const aiv::Vecf<3> &r, const aiv::Vecf<3> &center, const aiv::Vecf<3> &n, double phi){
    if (n.abs() < 1e-16) WRAISE("Transformation is incorrect");
    Vecf<3> r0 = r;
    if (phi > 1e-16){ r0 = rotate(n, -phi, r0); }
    if ((n^2).abs() > 1e-16){ r0 = rotate(Vctr(n[1], -n[0], 0.), acos(n[2]/n.abs()), r0); } else {
...
struct Cylinder: public BaseFigure{
    aiv::Vecf<3> boc; // центр нижней грани
    double R, H;    // радиус и высота
    aiv::Vecf<3> n; // направляющая вдоль оси z

    virtual Vecf<3> get_min() const {
        Vecf<3> tmp_min = rotmov_back(Vctr(-R,-R,0.), boc, n, 0.);
    ...
    virtual Vecf<3> get_max() const {
        Vecf<3> tmp_max = rotmov_back(Vctr(R,R,H), boc, n, 0.);
        for (int ix=-1;ix<=1;ix+=2) for (int iy=-1;iy<=1;iy+=2) for (int iz=0;iz<=1;iz+=1){
            tmp_max>>=rotmov_back(Vctr(R*ix,R*iy,H*iz), boc, n, 0.);
        }
        return tmp_max;
    }
    virtual bool check(const aiv::Vecf<3> &r) const {
...
struct Box: public BaseFigure{
    ...
    virtual Vecf<3> get_min() const {
        Vecf<3> tmp_min = rotmov_back(Vctr(-A*0.5,-B*0.5,0.), boc, n, phi);
        for (int ix=-1;ix<=1;ix+=2) for (int iy=-1;iy<=1;iy+=2) for (int iz=0;iz<=1;iz+=1){
    ...
    virtual Vecf<3> get_max() const {
        Vecf<3> tmp_max = rotmov_back(Vctr(A*0.5,B*0.5,H), boc, n, phi);
        for (int ix=-1;ix<=1;ix+=2) for (int iy=-1;iy<=1;iy+=2) for (int iz=0;iz<=1;iz+=1){
...
struct Sphere: public BaseFigure{
    aiv::Vecf<3> center; // центр
    double R;    // радиус

    virtual Vecf<3> get_min() const { return center - Vecf<3>(R); }
    virtual Vecf<3> get_max() const { return center + Vecf<3>(R); }
    virtual bool check(const aiv::Vecf<3> &r) const { return (r-center).abs()<=R; }
};
//------------------------------------------------------------------------------
Figure cylinder(const aiv::Vecf<3> bottom_origin_center, const aiv::Vecf<3> &n, double R, double H){
...
//------------------------------------------------------------------------------
Figure box(const aiv::Vecf<3> bottom_origin_center, const aiv::Vecf<3> &n, double phi, double A, double B, double H){
    Box* f = new Box;
    f->boc = bottom_origin_center;
...
//------------------------------------------------------------------------------
Figure cube(const aiv::Vecf<3> bottom_origin_center, const aiv::Vecf<3> &n, double phi, double A){
    Box* f = new Box;
    f->boc = bottom_origin_center;
...
//------------------------------------------------------------------------------
Figure spheroid(const aiv::Vecf<3> center, double R){
    Sphere* f = new Sphere;
